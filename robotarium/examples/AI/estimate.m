...
for ii = 1:1:N
    for jj = 1:1:N
        ...
        if ((ii ~= jj) && neighbors(ii, jj)) % ii is not jj and jj is ii's neighbor
            ...
            % ii is not jj and jj is ii's neighbor
            % haha jk i already have the consensus equation, its literallly
            % the line below
            
            % i need to consider the delta_est of jj because i know that
            % it'll be 0 always for the imposter but at the beginning
            % everything is 0 so thats kind of a conundrum 

            delta_temp = temperature_est(ii, 1) - temperature_est(jj, 1);
            delta_est(ii, 1) =  delta_est(ii, 1) - weight_consensus * (delta_temp);

            if(delta_est(jj, 1) ~= 0)
                
                if(delta_est(ii, 1) * delta_est(jj, 1) < 0)
                    delta_est(ii, 1) = delta_est(ii, 1) * -delta_est(jj, 1);
                else
                    delta_est(ii, 1) = delta_est(ii, 1) * delta_est(jj, 1);
                end
            end
