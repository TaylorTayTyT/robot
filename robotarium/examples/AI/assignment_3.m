...
for t = 1:time_step:total_iterations
    ...
    delta_temperature_est = estimate(temperature_est, neighbors, N, bad_robot);

    % Update sensor estimate by incorporating the amount of change
    temperature_est = temperature_est + (time_step * delta_temperature_est); 
    
    % Bad robot does its own thing
    temperature_est(bad_robot, 1) = malicious_estimate; 
    
    % Store everyone's history of temperature estimates for plotting purposes
    temperature_est_history = [temperature_est_history, temperature_est];
